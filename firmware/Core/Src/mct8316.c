...
HAL_StatusTypeDef MCT8316_Init(MCT8316 *mct8316) {
    ...
    status = MCT8316_Write(mct8316, MCT8316_REG_CTRL_2, MCT8316_CTRL_2_SDO_MODE | (3 << 1));  // PWM_MODE = 3 (Digital Hall Input Synchronous ASR/AAR)
    ...
    status = MCT8316_Write(mct8316, MCT8316_REG_CTRL_3, MCT8316_CTRL_3_PWM_100_DUTY_SEL |
                                                       MCT8316_CTRL_3_OVP_SEL |
                                                       MCT8316_CTRL_3_OVP_EN);
    if (status != HAL_OK) return status;
...
/* Private functions */
static HAL_StatusTypeDef MCT8316_Write(MCT8316 *mct8316, uint8_t address, uint8_t data){
	uint16_t message = (uint16_t) data;
	message |= (MCT8316_MSG_W << MCT8316_MSG_RW_BIT) | (address << MCT8316_MSG_ADDRESS_BIT);
	/* Compute parity */
	uint8_t parity = 0;
	uint16_t n = message;
	while (n)
	...
	if (parity) message |= (1 << MCT8316_MSG_PARITY_BIT);

	return HAL_SPI_Transmit(mct8316->hspi, (uint8_t *) &message, sizeof(message), MCT8316_SPI_TIMEOUT);
}

static HAL_StatusTypeDef MCT8316_Read(MCT8316 *mct8316, uint8_t address, uint8_t *data){
	HAL_StatusTypeDef status;
	uint16_t message = 0;
	message |= (MCT8316_MSG_R << MCT8316_MSG_RW_BIT) | (address << MCT8316_MSG_ADDRESS_BIT);
	/* Compute parity */
	uint8_t parity = 0;
	uint16_t n = message;
	while (n)
	...
	/* Transmit read message */
	status = HAL_SPI_Transmit(mct8316->hspi, (uint8_t *) &message, sizeof(message), MCT8316_SPI_TIMEOUT);
    if (status != HAL_OK) return status;

    /* Receive answer */
    status = HAL_SPI_Receive(mct8316->hspi, (uint8_t *) &message, sizeof(message), MCT8316_SPI_TIMEOUT);
    if (status != HAL_OK) return status;

    *data = (uint8_t) (message & 0xFF);
    mct8316->ic_status = (uint8_t) (message >> 8);
